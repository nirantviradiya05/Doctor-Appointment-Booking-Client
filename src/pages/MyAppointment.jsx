...
const MyAppointments = () => {
  ...
  const cancelAppointment = async (appointmentId) => {
    try {
      const { data } = await axios.post('/api/user/cancel-appointment', { appointmentId }, { headers: { token } });
      if (data.success) {
  ...
  const initPay = (order) => {
    const options = {
      ...
      handler: async (response) => {
        try {
          const { data } = await axios.post('/api/user/verify-razorpay', response, { headers: { token } });
          if (data.success) {
            getUserAppointments();
  ...
  const appointmentRazorpay = async (appointmentId) => {
    try {
      const { data } = await axios.post('/api/user/payment-razorpay', { appointmentId }, { headers: { token } });
      if (data.success) {
        initPay(data.order);
      }
    } catch (error) {
        ...
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_3fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={item._id || index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {!item.cancelled && !item.payment && <button onClick={() => appointmentRazorpay(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-indigo-500 hover:text-white transition-all duration-300'>Pay Online</button>}
